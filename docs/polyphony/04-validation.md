...
| Panel FE — the method's **second** limit (direction not in the data) | ✅ **stated honestly, then self-corrected** — education ⇄ income (236 countries) *survives* two-way FE (within +0.22, OOS +0.38) but its direction cannot be read from the data. Iter 61 read the symmetric lead-lag as the discriminator; **Iter 62 corrected that** — the three income→X survivors are *also* near-symmetric (reverse marginally stronger for all), so the lead-lag is **low-powered for persistent series** and cannot certify direction for any of them. A surviving within-correlation is **necessary but not sufficient** for a directional claim; direction rests on **manipulability** (income is an actionable lever, the reverse is not a policy channel), *not* a data lead-lag. Education stays out (two manipulable channels). `experiments/education_directionality.py` | Iters 61–62 |
| Panel FE — a **survivor** (generalization boundary) | ✅ the Preston curve (income → life expectancy, 252 countries) **keeps** a positive within-country corr (**+0.13**) under two-way FE where leakage collapsed to ≈0. A genuine within-unit mechanism survives — the instrument is not a "nothing works" machine; the aggregate-forecasting generalization has a boundary. `experiments/panel_validation.run_preston_panel` | Iter 51 |
...
| Macro⇄Health underpowering — **demonstrated** | ✅ the modern GDP era has **one** pandemic episode (COVID), so the coupling cannot be learned-on-one-and-tested-on-another (`identifiable_out_of_sample=False`); and the **annual** severity↔growth corr is **near zero** (−0.02; 2021 had the most deaths *and* recovery growth — a lockdown-vs-deaths timing confound). The synthetic keep is machinery; the real footing is genuinely thin. `experiments/real_macrohealth.py` | Iter 47 |
