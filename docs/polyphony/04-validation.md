...
| Panel FE — the method's **second** limit (direction-blindness) | ✅ **stated honestly** — education ⇄ income (236 countries) *survives* two-way FE (within +0.22, OOS +0.38) yet the within-country lead-lag is **symmetric** (schooling→income +0.37 vs income→schooling +0.38): a surviving within-correlation is **necessary but not sufficient** for a *directional* claim when causation is bidirectional. Deliberately **not** promoted to a reliable finding. `experiments/education_directionality.py` | Iter 61 |
